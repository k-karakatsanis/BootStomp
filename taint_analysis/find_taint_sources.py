...
class GetTaintSource:
    ...
    def display_methods(self):
        ...
        for function_index in range(50):
            ...
            if function_info.isValid:
                print('{}. {}({}) => {}'.format(function_index + 1,
                                                function_info.name,
                                                hex(function_info.address),
                                                function_info.xrefs))
    ...
    def display_strings_and_respective_caller_methods(self):
        ...
        for (str_idx, candidate_string) in enumerate(self.candidate_strings):
            print("{:d} => 0x{:X}: {}".format(str_idx, candidate_string.ea,
                                              str(candidate_string).strip()))

            for caller_method in str_refs[str_idx]:
                print("      |-- {} [0x{:X}]: 0x{:X}".format(caller_method.name,
                                                             caller_method.address,
                                                             caller_method.call_site))
                if "instr_" not in caller_method.name:
                    guard_analyze.traverse(caller_method.address,
                                           caller_method.call_site,
                                           candidate_string.ea)
        ...
        for taint_source_per_instance in guard_analyze.taint_sources_all_instances:
            ...
            taint_source.arg_cnt = guard_analyze.get_function_args_count(
                method_addr)
            taint_source.non_string_args = guard_analyze.get_non_string_args(
                method_addr)
    ...
    def heuristic_search_keywords_in_log_messages(self):
        ...
        if self.source_or_sink == 0:
            keywords = [
                ["mmc", "emmc", "sd", "flash", "oeminfo", "offset", "index",
                 "kernel", "ramdisk"],
                ["read", "head", "header"],
                ["error", "fail", "failed"]]
        ...
        for str_index in range(string_count):
            ...
            log_words = list(filter(None, log_words))
            ...
            for i in range(keyword_count):
                ...
                for j in range(len(keywords[i])):
                    if str(keywords[i][
                               j]) in log_words:  # Is the keyword found in the string?
                        match_count += 1
                        break

            for i in range(combined_keyword_count):
                group_size = len(combined_keywords[i])
                for j in range(log_words_count - group_size):
                    is_match = True
            ...
            for i in range(banned_keyword_count):
            ...
            if match_count == keyword_count and len(
                    log_message) <= max_log_message_length:
                self.candidate_strings.append(strings_found[
                                                  str_index])  # This is a string of our interest
                str_address = strings_found[
                    str_index].ea  # The address where the string lies at
                str_refs = DataRefsTo(
                    str_address)  # Enumerate the data references to the string
                ...
                for string_ref in str_refs:
                    ...
                    str_ref_by_func = get_func(
                        function_info.call_site)  # The method containing the instruction above
                    ...
                    if str_ref_by_func is not None:
                        ...
                        function_info.name = get_func_name(
                            function_info.address)  # Get the mnemonic function named by IDA
                    else:
                        function_info.name = "instr_" + str(hex(
                            function_info.call_site))  # Name the function in the form of "instr_<address>""
