...
class GetTaintSink:
    ...
    def __init__(self, top_xref_limit=50, min_arg_cnt=2, max_arg_cnt=3,
                 interfunction_level=1, max_bb_size=8, max_call_count=3):
    ...
    def display_methods(self, method_list=0, max_method_index=0):
        ...
        if method_list == 0:
            print("\nPotential memcpy():\n")
            for function_index, function_info in enumerate(
                    self.memcpy_like_functions):
                print('{}. {}({}, {}) => {}'.format(function_index + 1,
                                                    function_info.name,
                                                    hex(function_info.address),
                                                    function_info.arg_cnt,
                                                    function_info.xrefs))

        elif method_list == 1:
            print("\nSorted by Xref:\n")
            for function_index, function_info in enumerate(
                    helper.functions_info):
                print('{}. {}({}, {}) => {}'.format(function_index + 1,
                                                    function_info.name,
                                                    hex(function_info.address),
                                                    function_info.arg_cnt,
            ...
            for function_index in range(max_method_index):
                ...
                print('{}. {}({}, {}) => {}, {}, {}'.format(function_index + 1,
                                                            ...
                                                            function_info.arg_cnt,
                                                            function_info.xrefs,
                                                            function_info.isPotentialMemcpy,
                                                            function_info.isMemcpy))
            ...
            for function_index, function_info in enumerate(
                ...
                if function_info.isPotentialMemcpy:
                    print('{}. {}({}, {}) => {}'.format(function_index + 1,
                                                        function_info.name, hex(
                            function_info.address), function_info.arg_cnt,
                                                        function_info.xrefs))
        else:
            print("\nmemcpy()\n")
            for function_index, function_info in enumerate(
    ...
    def mark_potential_memcpy(self, max_method_index=0):
        ...
        for function_index in range(max_method_index):
            function_info = helper.functions_info[function_index]
            function_info.arg_cnt = helper.get_function_args_count(
                function_info.address)
            if self.min_arg_cnt <= function_info.arg_cnt <= self.max_arg_cnt:
    ...
    def mark_memcpy(self, max_method_index=0):
        ...
        for function_index in range(max_method_index):
            function_info = helper.functions_info[function_index]
            if function_info.isPotentialMemcpy:
                function_info.isMemcpy = self.is_memcpy_function(
                    function_info.address)
    ...
    def is_memcpy_block(self, basic_block, external_calls, func_start_addr,
                        func_end_addr, interfunction_level=0):
        bb_size = 0
        is_bb_memcpy = False
        load_mnemonics = ["LDR", "LDRH", "LDRB"]
        store_mnemonics = ["STR", "STRH", "STRB"]
        branch_mnemonics = ["B", "BL", "BX", "BLX", "BXJ", "BNE", "BGE", "BGT",
                            "BLE", "BLT"]
        registers = ["LR"]
        memcpy_pattern = [load_mnemonics, store_mnemonics]
        opcode_type = 0
        instr_addr = basic_block.start_ea
        bb_start = ida_name.get_ea_name(basic_block.start_ea)
        ...
        while instr_addr < basic_block.end_ea:
            ...
            if mnemonics in branch_mnemonics:
                ...
                if func_start_addr <= branch_tgt_addr <= func_end_addr:
                    if not is_bb_memcpy:
                ...
                # Inter-procedural jump
                    ...
                    if not is_bb_memcpy:
                        if branch_tgt_addr not in external_calls:
                            if self.is_memcpy_function(branch_tgt_addr,
    ...
    def is_memcpy_function(self, instr_addr, interfunction_level=0):
        ...
        for block in flow_chart:
            is_bb_memcpy = self.is_memcpy_block(block, external_calls,
