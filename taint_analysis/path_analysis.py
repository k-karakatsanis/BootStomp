...
class PathAnalyze:
    ...
    def write_function_info(self):
        with open('function_info.txt', 'w') as finfo:
            finfo.write(
                "# method_name, method_addr, method_size, condition_stmt, loop_stmt\n\nbase_addr:\n0x%X\n\nfunctions:\n" % get_segm_start(inf_get_min_ea()))
            for func_info in self.all_func_info:
                finfo.write(
                    func_info.name + ", " + "0x%X" % func_info.addr + ", " + str(
                        func_info.size) + ", " + str(
                        func_info.cond_stmt) + ", " + str(
                        func_info.loop_stmt) + "\n")
    ...
    def populate_function_info(self):
        ...
        for my_function in functions:
            func_info = FuncInfo()
            func_info.name = get_func_name(my_function)
            func_info.addr = my_function
            func_info.size = my_function
            print("Analyzing %s at 0x%X" % (func_info.name, func_info.addr))
            func = get_func(my_function)
