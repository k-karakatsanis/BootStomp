...
def find_instr(instr, seg_start=SegStart(ea), seg_end=SegEnd(ea)):
    ...
    while addr <= seg_end:
        ...
        if instr_belogs_to_func is not None:
            instr_func_addresses.add(get_func_name(
                instr_belogs_to_func.start_ea))  # Get the mnemonic function named by IDA
        else:
            instr_func_addresses.add("instr_" + str(
                instr_address))  # Name the function in the form of "instr_<address>""
